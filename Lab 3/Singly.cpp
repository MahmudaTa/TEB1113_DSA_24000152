...
public:
    string name;
    Node* next_ptr; // Changed from int* to Node*

    Node(string name, Node* next_ptr = nullptr) { // Constructor with default nullptr
        this->name = name;
...
public:
    LinkedList(Node* head = nullptr) { // Constructor takes Node* and initializes tail
        this->head = head;
        this->tail = head;
    ...
    void add_element(Node* node) {
        if (!node) return; // Check for null pointer
    ...
    void display_list() { // Added parentheses and implementation
        Node* current = head;
        if (!current) {
    ...
    void delete_by_value(string val) {
        ...
        if (current->next_ptr) { // If value found
            Node* temp = current->next_ptr;
            current->next_ptr = temp->next_ptr;
            if (temp == tail) tail = current; // Update tail if deleting last node
            delete temp;
        }
    }
